...
class CreateLoanRequestTest(APITestCase):
    def setUp(self):
        ...
        self.loan_request = {
            ...
            "amount": self.loan_type.max_amount,
            "term": self.loan_type.loan_terms.first().id,
            "loan_type": self.loan_type.id,
        }

    def test_create_valid_loan_request(self):
        self.loan_request["amount"] = self.loan_request["amount"] - 1
        response = self.client.post(self.url, self.loan_request)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_create_loan_request_with_amount_eq_loan_type_min_amount(self):
        self.loan_request["amount"] = self.loan_type.min_amount
        response = self.client.post(self.url, self.loan_request)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_create_loan_request_with_amount_eq_loan_type_max_amount(self):
        self.loan_request["amount"] = self.loan_type.max_amount
        response = self.client.post(self.url, self.loan_request)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_create_loan_request_with_amount_lt_loan_type_min_amount(self):
        self.loan_request["amount"] = self.loan_type.min_amount - 1
        response = self.client.post(self.url, self.loan_request)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(str(response.data), self.expected_error)

    def test_create_loan_request_with_amount_gt_loan_type_max_amount(self):
        self.loan_request["amount"] = self.loan_type.max_amount + 1
        response = self.client.post(self.url, self.loan_request)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(str(response.data), self.expected_error)
